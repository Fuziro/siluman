...
def sukses(no1,pro,nam):
  print "     %s[%s%s%s] [%s Berhasil %s] %sBerhasil, kirim kentut %s dari %s%s %ssended"%(pu,ku,no1,pu,hi,pu,pu,pro,ku,nam,hi)
#--------------------------------MAIN---------------------------------------------------
def main():
  print "%s[%s!%s] %sKorban locked >> %s%s"%(pu,me,pu,pu,ku,"+62"+nom)
  t = 1
...
def logo():
 ...
 | |\/| | |) | \__ \ '_ \/ _` | '  \| '  \/ -_) '_(_-< %sGithub %sgithub.com/Fuziro
 |_|  |_|___/  |___/ .__/\__,_|_|_|_|_|_|_\___|_| /__/ %sTeam %sAshabul Coffe%s
                   |_|                                 %sDusun Sidomakmur"""%(qu,pu,ku,qu,pu,ku,qu,pu,ku,qu,qu)
#-------------------------------Input Function------------------------------------------
def input():
  global nom
  nom = raw_input("%s[%s?%s] %sMasukkan nomor Korban (8888xx) : "%(pu,me,pu,pu))
  if len(nom) < 5:
    print "%s[%s!%s] %sMasukkan nomor Korban dengan benar!!"%(pu,me,pu,me)
    input()
    ...
    global jum
    jum = int(raw_input("%s[%s?%s] %sMasukkan jumlah Kentut : "%(pu,me,pu,pu)))
    main()
#-------------------------------SPAM Function-------------------------------------------
def asakita():
...
def jumpstart():
 ...
 if json.loads(tes.text)["data"] == None:
  gagal("100","sms","jumpstart")
 else:
  sukses("0","sms","jumpstart")
def kinimart():
